...
typedef struct _ffmpeg_ao_t_ 
{
    int (*ffmpeg_ao_open)(AUDIO_DRIVER_HANDLE* outHandle, uint32_t sampleRate, int format, int channels);
    int (*ffmpeg_ao_close)(AUDIO_DRIVER_HANDLE handle);
    uint32_t (*ffmpeg_ao_latency)(AUDIO_DRIVER_HANDLE handle);
    int (*ffmpeg_ao_start)(AUDIO_DRIVER_HANDLE handle);
    int (*ffmpeg_ao_write)(AUDIO_DRIVER_HANDLE handle, void *buffer, int buffer_size);
    int (*ffmpeg_ao_flush)(AUDIO_DRIVER_HANDLE handle);
    int (*ffmpeg_ao_stop)(AUDIO_DRIVER_HANDLE handle);
    int (*ffmpeg_ao_reload)(AUDIO_DRIVER_HANDLE handle);    
} ffmpeg_ao_t, *pffmpeg_ao_t;

extern int ffmpeg_ao_register(ffmpeg_ao_t* ao);
